...
**Article conventions** (see `packages/ui/DESIGN.md` → *Bleed / vyčuhující prvky*, *RelatedArticles*, *New-article checklist*): every real article ends with a `Doporučujeme` block (`<RelatedArticles heading="Doporučujeme" />`) and should include one mid-article `<SupportBanner float="right" />` in a text-heavy paragraph (the full-width end banner is added automatically). Side asides that "bleed" past the 800px reading column — `<Figure side="right">`, ```` ```infobox … right ````, `RelatedArticles position="right"` ("Napsali jsme"), `SupportBanner float` — all share the `--dt-bleed-*` tokens in `apps/web/app/globals.css` (48% width / -12% outer bleed). Text and headings flow-wrap around them until they end (H2/H3 carry no `clear`).
