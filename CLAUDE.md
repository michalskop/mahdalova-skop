...
**MDX components** available in article markdown (registered in `ArticleRenderer`): `MediaBox`, `InfoBox`, `FlourishEmbed`, `ScrollyTelling`, `Timeline`, `RelatedArticles`, `PartyFace`, `MotionsStancesTable`, `Figure`, `SupportBanner`.

**Article conventions** (see `packages/ui/DESIGN.md` → *Bleed / vyčuhující prvky*, *RelatedArticles*, *New-article checklist*): every real article MUST contain, without being asked: (1) an inline related box `<RelatedArticles preset="sidebar" position="right" heading="🔻🔻🔻" … />` in the **first quarter**, (2) an inline `<SupportBanner float="left" />` in the **upper half** (lower than and in a different stretch than the related box), (3) the read-more block `<RelatedArticles heading="🔻🔻🔻" />` as the last markdown element (the full-width SupportBanner under it is added automatically – never add it by hand). Both floats go only where ≥3–4 plain paragraphs (no heading/box/chart/table) wrap them fully; if a short or graphics-heavy text has no such spot, drop the inline related and use a non-float SupportBanner. The heading of related-article boxes is always `🔻🔻🔻`, never the old "Doporučujeme" / "Napsali jsme" text. Canonical rule: `docs/redakcni-styl/REDAKCNI_MANUAL.md` → *Povinné vložené prvky v každém článku*. The Google button „Chci víc DataTimes.cz ve zprávách Googlu" (`PreferredSource`) is rendered automatically in every article byline – never add it by hand. Side asides that "bleed" past the 800px reading column — `<Figure>`, ```` ```infobox … right ````, `RelatedArticles position="right"` ("Napsali jsme"), `SupportBanner float` — all share the `--dt-bleed-*` tokens in `apps/web/app/globals.css` (48% width / -12% outer bleed). **Side convention:** SupportBanner → **left**; side InfoBox / "Napsali jsme" → **right**; photos (`<Figure>`) **alternate** by text flow; big boxes → full-width; mobile (≤768px) → everything full-width. Don't put a left and a right float in the same short paragraph cluster. Text and headings flow-wrap around them until they end (H2/H3 carry no `clear`).

**Photos rule:** every photograph in article text is a `<Figure src side alt caption />` (floated, click-to-enlarge lightbox that repeats the caption on a translucent ink-blue strip) – never a bare `![](…)` + italic caption line. The `caption` string supports inline `[link](url)`, `**bold**`, `_italic_`; no straight `"` inside it. Place it at the start of the text-heavy paragraph it belongs to (not before a box/table/chart). Exceptions that stay full-width `![](…)`: charts, maps, tables, document/social-media screenshots, text graphic cards, header banners; a cohesive photo set is a ```` ```fotogalerie ```` block. Canonical: `docs/redakcni-styl/REDAKCNI_MANUAL.md` → *Fotografie a obraz*.
