...
function BarRow({ label, value, max, color = 'var(--mantine-color-brandNavy-6)', suffix = '' }: { label: string; value: number; max: number; color?: string; suffix?: string }) {
  return (
    <Box style={{ display: 'grid', gridTemplateColumns: '96px 1fr 92px', gap: 12, alignItems: 'center', margin: '12px 0' }}>
      ...
      <Box h={24} bg="var(--mantine-color-background-3)" style={{ border: '1px solid var(--mantine-color-background-6)', borderRadius: 999, overflow: 'hidden' }}>
    ...
    <Container size="lg" bg="background.2" maw="1200px" w="100%" p={0} m="0 auto">
      <Box px={{ base: 18, md: 42 }} py={{ base: 28, md: 48 }} bg="brandRoyalBlue.8" c="background.0">
        <Stack gap="lg">
          ...
          <Title order={1} style={{ fontSize: 'clamp(2.4rem, 6vw, 5.6rem)', lineHeight: 0.95, maxWidth: 900 }}>
            Festival se dá počítat i před závěrečnými titulky.
          </Title>
          <Text size="xl" maw={780} c="background.2">
      ...
      <Box px={{ base: 16, md: 24 }} py={{ base: 24, md: 34 }}>
        <SimpleGrid cols={{ base: 1, sm: 2, md: 4 }} spacing="md">
          {[
            ...
            <Paper key={label} p="lg" radius={8} withBorder bg="background.1">
      ...
      <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md" px={{ base: 16, md: 24 }} pb={24}>
        <Paper p="lg" radius={8} withBorder bg="background.1">
          <Title order={2} mb="xs" >Vstupenky: finále ročníků a letošní průběžný stav</Title>
          ...
          <BarRow label="2026*" value={current2026.tickets} max={maxTickets} color="var(--mantine-color-brandOrange-6)" />
        ...
        <Paper p="lg" radius={8} withBorder bg="background.1">
          <Title order={2} mb="xs" >Stav 2026 proti finálnímu roku 2025</Title>
          <Text c="dimmed" mb="md">Srovnání je orientační: rozběhnutý ročník proti uzavřenému.</Text>
          <BarRow label="Vstupenky" value={ticketShare2026} max={100} color="var(--mantine-color-brandOrange-6)" suffix="%" />
          <BarRow label="Projekce" value={screeningsShare2026} max={100} color="var(--mantine-color-brand-6)" suffix="%" />
          <BarRow label="Fest.Passy" value={passesShare2026} max={100} color="var(--mantine-color-brandTeal-6)" suffix="%" />
          <BarRow label="Novináři" value={journalistsShare2026} max={110} color="var(--mantine-color-brandTeal-6)" suffix="%" />
          <BarRow label="Industry" value={industryShare2026} max={110} color="var(--mantine-color-brandTeal-6)" suffix="%" />
          <Text mt="md">Fest.Passy, novináři a filmoví profesionálové jsou už kolem loňského finále nebo nad ním.</Text>
        </Paper>

        <Paper p="lg" radius={8} withBorder bg="background.1">
          <Title order={2} mb="xs" >Peníze: rozpočet a stopa návštěvníků</Title>
          <Text c="dimmed" mb="md">Údaje 2026 podle průběžného stat stavu.</Text>
          <BarRow label="Rozpočet" value={current2026.budgetMil} max={current2026.spendingMil} color="var(--mantine-color-brandNavy-6)" suffix="mil." />
          <BarRow label="Útrata" value={current2026.spendingMil} max={current2026.spendingMil} color="var(--mantine-color-brandOrange-6)" suffix="mil." />
        ...
        <Paper p="lg" radius={8} withBorder bg="background.1">
          <Title order={2} mb="xs" >Financování rozpočtu 2026</Title>
          <Text c="dimmed" mb="md">Při rozpočtu 250 mil. Kč a poměru 80/20.</Text>
          <BarRow label="Sponzoři" value={sponsorMil2026} max={current2026.budgetMil} color="var(--mantine-color-brandOrange-6)" suffix="mil." />
          <BarRow label="Veřejné" value={publicMil2026} max={current2026.budgetMil} color="var(--mantine-color-brandNavy-6)" suffix="mil." />
      ...
      <Box px={{ base: 16, md: 24 }} pb={34}>
        <Paper p="lg" radius={8} withBorder bg="brandRoyalBlue.8" c="background.0">
          <Title order={2} mb="md" >Věty do vysílání</Title>
      ...
      <Box px={{ base: 16, md: 24 }} pb={44}>
        <Title order={2} mb="sm" >Pozor na formulace</Title>
        <Stack gap={6}>
