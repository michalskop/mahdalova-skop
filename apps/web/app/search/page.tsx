...
export default function SearchPage() {
  ...
  return (
    <Container size="md" maw="928px" py="xl">
      <Title order={1} mb="lg">Hledat</Title>

      <TextInput
        size="md"
        placeholder={ready ? 'Hledat v článcích…' : 'Načítání…'}
        value={query}
      ...
      {!ready && !timedOut && (
        <Text c="dimmed" size="sm">Načítání vyhledávání… {countdown}</Text>
      )}
      {timedOut && !ready && (
        <Text c="dimmed" size="sm">Vyhledávání není k dispozici.</Text>
      )}
      {ready && loading && (
        <Text c="dimmed" size="sm">Hledám…</Text>
      )}
      {ready && !loading && query.trim() && results.length === 0 && (
        <Text c="dimmed" size="sm">Žádné výsledky pro &bdquo;{query}&ldquo;</Text>
      )}
