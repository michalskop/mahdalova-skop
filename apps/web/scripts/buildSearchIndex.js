...
const articlesDir = path.join(process.cwd(), 'app/clanek/_articles');
...
fs.mkdirSync(path.join(tempDir, 'clanek'), { recursive: true });
...
<div>${body}</div>
  ...
  fs.writeFileSync(path.join(tempDir, 'clanek', `${slug}.html`), html);
  count++;
}
